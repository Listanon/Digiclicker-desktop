  ...
  const replaceDiscordText = (input) => {
    try {
      output = input.replace(/{caught}/g, App.game.party.caughtPokemon.length || 0)
                    ...
                    .replace(/{underground_levels_cleared}/g, App.game.statistics.junkyardLayersMined().toLocaleString('en-US'))
                    .replace(/{underground_items_found}/g, App.game.statistics.junkyardItemsFound().toLocaleString('en-US'))
