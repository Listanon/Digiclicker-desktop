...
function createWindow() {
  ...
  mainWindow.setTitle('DigiClicker');

  // Check if we've already downloaded the data, otherwise load our loading screen
  if (fs.existsSync(`${dataDir}/Digiclicker-master/docs/index.html`)) {
    mainWindow.loadURL(`file://${dataDir}/Digiclicker-master/docs/index.html`);
  } else {
    mainWindow.loadURL(`file://${__dirname}/Digiclicker-master/docs/index.html`);
  }
...
function createSecondaryWindow() {
  ...
  newWindow.setTitle('DigiClicker (alternate)');

  // Check if we've already downloaded the data, otherwise load our loading screen
  if (fs.existsSync(`${dataDir}/Digiclicker-master/docs/index.html`)) {
    newWindow.loadURL(`file://${dataDir}/Digiclicker-master/docs/index.html`);
  } else {
    newWindow.loadURL(`file://${__dirname}/Digiclicker-master/docs/index.html`);
  }
...
if (!isMainInstance) {
  ...
  */
  const isNewerVersion = (version) => {
    ...
    return version.localeCompare(currentCrobatVersion, undefined, { numeric: true }) === 1;
  }

  const downloadUpdate = async (initial = false) => {
    const zipFilePath = `${dataDir}/update.zip`;
    const file = fs.createWriteStream(zipFilePath);
    https.get('https://codeload.github.com/listanon/digiclicker/zip/master', async res => {
      let cur = 0;
      try {
      ...
      res.pipe(file).on('finish', async () => {
        ...
        const extracted = zip.extractEntryTo('Digiclicker-master/docs/', `${dataDir}`, true, true);
        ...
        const userResponse = dialog.showMessageBoxSync(mainWindow, {
          title: 'DigiClicker - Update success!',
  ...
  const downloadUpdateFailed = () => {
    ...
    const userResponse = dialog.showMessageBoxSync(mainWindow, {
      type: 'error',
      title: 'DigiClicker - Update failed!',
  ...
  const checkForUpdates = () => {
    const request = https.get('https://raw.githubusercontent.com/Listanon/digiclicker/master/package.json', res => {
      let body = '';
      ...
      res.on('end', () => {
        let data = {version:'0.0.0', crobatVersion:'0.0.0'};
        try {
          data = JSON.parse(body);
          newVersion = data.version;
          newCrobatVersion = data.crobatVersion;
          const newVersionAvailable = isNewerVersion(data.crobatVersion);
  ...
  const shouldUpdateNowCheck = () => {
    const userResponse = dialog.showMessageBoxSync(mainWindow, {
      title: 'DigiClicker - Update available!',
  ...
  try {
    ...
    currentVersion = JSON.parse(fs.readFileSync(`${dataDir}/Digiclicker-master/docs/package.json`).toString()).version;
    currentCrobatVersion = JSON.parse(fs.readFileSync(`${dataDir}/Digiclicker-master/docs/package.json`).toString()).crobatVersion;
  ...
  try {
    autoUpdater.on('update-downloaded', () => {
      const userResponse = dialog.showMessageBoxSync(mainWindow, {
        title: 'DigiClicker - Client Update Available!',
